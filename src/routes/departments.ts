...
// Delete a department (blocked when subjects still reference it)
    ...
    try {
        ...
        if (Number(subjectCount) > 0) {
            return res.status(409).json({
                error: "Cannot delete a department that still has subjects. Reassign or delete its subjects first.",
            });
        }
