...
subjectsRouter.post("/", async (req, res) => {
    try {
        ...
        if (!departmentId || !name || !code) {
            return res
                .status(400)
                .json({ error: "departmentId, name and code are required" });
        }
...
// Delete a subject (blocked when classes still reference it)
    ...
    try {
        ...
        if (Number(classCount) > 0) {
            return res.status(409).json({
                error: "Cannot delete a subject that still has classes. Delete its classes first.",
            });
        }
